...
public final class ConfigBeanImpl {
    ...
    public static <T> T createInternal(Config config, Class<T> clazz) {
        ...
        try {
            ...
            throw new ConfigException.BugOrBroken("Could not get bean information for class " + clazz.getName(), e);
        }
        ...
        try {
            ...
            throw new ConfigException.BugOrBroken(clazz.getName() + " needs a public no-args constructor to be used as a bean", e);
        } catch (IllegalAccessException e) {
            throw new ConfigException.BugOrBroken(clazz.getName() + " getters and setters are not accessible, they must be for use as a bean", e);
        } catch (InvocationTargetException e) {
            throw new ConfigException.BugOrBroken("Calling bean method on " + clazz.getName() + " caused an exception", e);
        }
    }
    ...
    private static Object getValue(Class<?> beanClass, Type parameterType, Class<?> parameterClass, Config config,
        ...
        if (parameterClass == Boolean.class || parameterClass == boolean.class) {
            ...
            if (typeArgs[0] != String.class || typeArgs[1] != Object.class) {
                throw new ConfigException.BugOrBroken("Bean property '" + configPropName + "' of class " + beanClass.getName() + " has unsupported Map<" + typeArgs[0] + "," + typeArgs[1] + ">, only Map<String,Object> is supported right now");
            }
            ...
            throw new ConfigException.BugOrBroken("Bean property " + configPropName + " of class " + beanClass.getName() + " has unsupported type " + parameterType);
        }
    }
    ...
    private static Object getListValue(Class<?> beanClass, Type parameterType, Class<?> parameterClass, Config config, String configPropName) {
        ...
        if (elementType == Boolean.class) {
            ...
            throw new ConfigException.BugOrBroken("Bean property '" + configPropName + "' of class " + beanClass.getName() + " has unsupported list element type " + elementType);
        }
    }
